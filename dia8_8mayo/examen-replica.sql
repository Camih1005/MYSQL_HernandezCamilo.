...
select p.nombre as profesor from profesor as p left join asignatura as a on p.id_profesor = a.id_profesor 
...
select g.nombre,count(g.id_grado) as cantidad
from grado as g join asignatura as a on g.id_grado = a.id_grado
group by g.id_grado  having cantidad  >= 40;
...
select g.nombre as grado,SUM(a.creditos) as cant_creditos 
from asignatura a 
...
select count(a.id_alumno) from alumno_se_matricula_asignatura as a; 



select * from asignatura where nombre like "s%"
